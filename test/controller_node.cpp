 ...
 *
 */
TEST(kal_controller_ros_tool, testControllerNode) {
    ...
    // The trajectory time stamps in distances imply a velocity of 1m/s
    EXPECT_DOUBLE_EQ(controlCommand.drive.speed, 1);
    ...
    EXPECT_LE(controlCommand.drive.steering_angle, 0.3);
}
 ...
 *
 */
TEST(kal_controller_ros_tool, testControllerNodeNoTrajectory) {
    // Create test publisher and subscriber
    ros::NodeHandle handler;
    Listener<ackermann_msgs::AckermannDriveStamped> controlCommandListener("/rostest/control_command", handler);
 ...
 *
 */
TEST(kal_controller_ros_tool, testControllerNodeOldTrajectory) {
    // Create test publisher and subscriber
    ros::NodeHandle handler;
    ros::Publisher trajectoryAdvertiser = handler.advertise<nav_msgs::Path>("/rostest/trajectory", 1);
    Listener<ackermann_msgs::AckermannDriveStamped> controlCommandListener("/rostest/control_command", handler);
 ...
 *
 */
TEST(kal_controller_ros_tool, testControllerNodeShortTrajectory) {
    // Create test publisher and subscriber
    ros::NodeHandle handler;
    ros::Publisher trajectoryAdvertiser = handler.advertise<nav_msgs::Path>("/rostest/trajectory", 1);
    Listener<ackermann_msgs::AckermannDriveStamped> controlCommandListener("/rostest/control_command", handler);
